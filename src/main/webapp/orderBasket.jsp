	...
	<form action="basketInsertOrder.do" method="post">
		<table id="customers" class="custom">
			...
			<tr>
				...
				<td>배송비 : 2500</td>
				<td colspan="2">총가격 : <%=order_total + 2500%></td>
			</tr>
		</table>
		<table id="customers" class="custom">
			<tr>
		...
		<div class=container stlye="margin:10px 0 0 0;">
			<div class=row>
				<div class=col>
				<input type="hidden" name="member_email" value="<%=member_email%>">
				<input type="hidden" name="order_date" value="<%=today%>"> <input
					type="hidden" name="sell_month" value="<%=today2%>"> <input
					type="hidden" name="order_state" value="준비중"> <input
					type="hidden" name="order_total" value="<%=order_total%>">
			
			<input class="btn btn-success" type="submit" value="구매하기">
			</div>
			<div class=col>
				<input  class="btn btn-success" type="button" value="취소하기">
			</div>
		</div>
		</div>
	</form>
